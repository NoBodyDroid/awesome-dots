...
local taglist_buttons = gears.table.join( awful.button({ }, 1, function(t) t:view_only() end), awful.button({ modkey }, 1, function(t) 
...
klayout = {{{{{{{widget = wibox.widget.imagebox,image = "/home/nbd/.config/awesome/icons/keyboard.svg"},widget = wibox.container.place},bottom = 2,top =2,right = 1,left = 2,
    ...
    layout = wibox.layout.fixed.horizontal},widget = wibox.container.margin,bottom = 2,top =2,right = 2,left = 2}, bg = "#F05454",fg = "#000000",widget = wibox.container.background
...
mylauncher = {s.mylauncher,widget = wibox.container.margin,top = 2,bottom =2 ,left = 10 }
--**********************************************
...
layout = wibox.layout.fixed.horizontal}
--**********************************************
sysmonitor = {{{{{myram,widget = wibox.container.place},spacing = 9,
            {gpu,widget = wibox.container.place},spacing = 9,
            {cpu,widget = wibox.container.place},spacing = 7,
            layout = wibox.layout.fixed.horizontal},widget = wibox.container.margin,left = 3,right =3 } ,bg = "#ACB4C3",fg = "#000000",  widget = wibox.container.background,
            shape = function(cr, width, height) gears.shape.rounded_rect(cr, width, 30, 5)end}, widget = wibox.container.margin,top = 3}
--**********************************************
...
--rofi start && clipboard -----------------------------------------
awful.key({ modkey },"space",function () awful.util.spawn('rofi -display-combi "" -combi-modi window,drun -show combi -show-icons') end,{description = "run rofi", group = "launcher"}),
awful.key({ modkey,"Mod1" },"space",function () awful.util.spawn('clipmenu') end,{description = "run rofi", group = "launcher"}),
awful.key({ modkey,"Mod1","Control" },"space",function () awful.util.spawn('rofi -show file-browser-extended -file-browser-show-hidden') end,{description = "run rofi", group = "launcher"}),
-- resizing arrow -----------------------------------------
...
--**********************************************
--toggle fullscreen -----------------------------------------
awful.key({ modkey }, "j",  function (c) c.fullscreen = not c.fullscreen c:raise() end, {description = "toggle fullscreen", group = "client"}),
--killapps --------------------------------------------------
...
--Rules
awful.rules.rules = {
{ rule = { }, properties = { border_width = beautiful.border_width, border_color = beautiful.border_normal, focus = awful.client.focus.filter, 
    raise = true, keys = clientkeys, buttons = clientbuttons, screen = awful.screen.preferred, placement = awful.placement.no_overlap+awful.placement.no_offscreen}},
-- Floating clients -----------------------------------------
{ rule_any = { instance = { "DTA", "copyq", "pinentry",},
    ...
    name = {"Event Tester",}, role = {"AlarmWindow","ConfigManager","pop-up",}}, properties = { floating = true }},
--Tag Rules
{ rule = { class = "Firefox" },
...
{ rule = { class = "GLava" },properties = {focusable=false, ontop=true, skip_taskbar=true,},
...
awful.spawn.with_shell("~/.config/awesome/script/Glava.sh")
--====================================
--====================================
-- Custom Commands 
...
client.connect_signal("property::maximized", function(c) if c.maximized then c.maximized = false end end)
--====================================
--====================================
